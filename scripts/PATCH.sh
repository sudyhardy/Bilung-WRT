...
patch_signature_check() {
    ...
    elif [ -f "distfeeds.conf" ]; then
        sed -i '\|option check_signature| s|^|#|' distfeeds.conf || true
    else
        log "WARNING" "No repositories.conf or distfeeds.conf found; skipping signature patch"
    fi
}

...
download_packages() {
    ...
    for entry in "${package_list[@]}"; do
        ...
        if [[ "$base_url" == *"api.github.com"* ]]; then
            ...
            download_url=$(echo "$file_urls" | grep -E '\.(ipk|apk)$' | grep -Ei "/${package_stem}([-_]|[^/])*" | sort -V | tail -1)
        fi
        ...
        if [[ "$base_url" != *"api.github.com"* ]]; then
            ...
            local patterns=(
                "${package_stem}[-_][^\"]*\.(ipk|apk)"
                "${package_stem}[^\"]*\.(ipk|apk)"
            )

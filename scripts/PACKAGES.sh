...
REPOS+=(
    ...
    ["GSPOTX2F"]="https://github.com/gSpotx2f/packages-openwrt/raw/master/${VEROP}"
...
verify_packages() {
    ...
    for package in "${package_list[@]}"; do
        ...
        local pkg_stem="${pkg_name%_}"
        if ! find "$pkg_dir" \( -name "${pkg_stem}*.ipk" -o -name "${pkg_stem}*.apk" \) -print -quit | grep -q .; then
